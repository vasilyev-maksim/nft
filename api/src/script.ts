...
const collection = generator.getCollections()[0];

const outFile = new File('../../client/src/collection.json');
outFile.writeJson({
  categories: collection.getCategories().map(x => {
    return {
      ...x.toJSON(),
      layers: x.getAllLayers().map(l => ({
        ...l.toJSON(),
        svg: l.getSvgBody(),
      })),
    };
